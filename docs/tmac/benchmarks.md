...
## Speculative Decoding (2026-03-13 to 2026-03-16)

Explored speculative decoding on RDNA3 with T-MAC. Key discovery: a 6x MMQ performance
cliff at batch=17 that was blocking high n_draft values. Fixed in commit `bdc2cb438`.

### Configuration

**Config B (Dual-GPU Device-Pinned)** — best performing setup:
- GPU 0: Target model only (full bandwidth isolation)
- GPU 1: Draft model only
- CLI: `HIP_VISIBLE_DEVICES=0,1 llama-speculative -m <target> -md <draft> -dev ROCm0 -devd ROCm1 --draft <n_draft> -ngl 99`

Config B eliminates cache interference between draft and target models.

### Validated Results

**Config B — Dual GPU (N=5, code generation, temp=0):**

| Draft → Target | n_draft | Median t/s ± SD | Baseline t/s | Speedup | alpha |
|----------------|--------:|----------------:|-------------:|--------:|------:|
| Gemma 4B Q4_K → 27B Q6_K | 15 | **91.87 ± 0.15** | 29.9 (single) | **+207%** | 84.7% |
| Llama 1B Q4_K → 70B IQ2_XXS | 15 | **58.12 ± 0.08** | 22.9 (row-split) | **+153%** | ~80% |
| Gemma 4B Q4_K → 27B Q4_K_M | 5 | 45.8 | 36.7 | +24.8% | 84.4% |

Gemma 91.87 t/s: CV=0.16% (N=5). Llama 70B 58.12 t/s: CV=0.14% (N=5, post-reboot clean VRAM).

**Config A — Single GPU (single-run, code generation, temp=0):**

| Draft → Target | n_draft | Spec t/s | Baseline t/s | Delta | alpha |
|----------------|--------:|---------:|-------------:|------:|------:|
| Llama 1B Q4_K → 8B Q5_K | 5 | 154.1 | 104.1 | **+48.0%** | 82.4% |
| Gemma 4B Q4_K → 12B Q4_K | 5 | 78.7 | 70.6 | +11.5% | 86.7% |

**T-MAC effect under speculative decoding (4-way comparison):**

| Configuration | Llama 8B Code t/s | Gemma 12B Code t/s |
|---------------|------------------:|-------------------:|
| Spec + T-MAC | 154.1 | 78.7 |
| Spec + Stock | 144.2 | 72.2 |
| NoSpec + T-MAC | 104.1 | 70.6 |
| NoSpec + Stock | 97.4 | 63.8 |

T-MAC benefit persists under speculative decoding: +6.9% (Llama), +9.0% (Gemma).

### Model Pair Compatibility

| Family | Best Pair | Result | Notes |
|--------|-----------|--------|-------|
| **Gemma 3** | 4B → 27B Q6_K | **91.87 t/s (+207%)** | Best overall — high alpha, fast draft |
| **Llama 3.x** | 1B → 70B IQ2_XXS | **58.12 t/s (+153%)** | Config B required (70B needs single-GPU BW) |
| Llama 3.x | 1B → 8B | +48% (Config A) | 1B draft faster than 3B despite lower alpha |
| **Qwen 3** | 4B → 30B-A3B | alpha=79.4% (code) | Works (non-thinking), but MoE target too fast for net gain |
| Qwen 3 | 4B → 8B/14B | alpha 45–66% | Not profitable (low alpha) |
| **Qwen 3.5** | Any pair | **alpha 0.5–2%** | **Broken** — thinking tokens diverge completely |
| **Mistral** | Any pair | **Fails** | Tokenizer mismatch across sub-families |

**Key:** GGUF source matters — Gemma Q6_K from one provider had 6415 special tokens vs 6414
in Q4_K_M from another. llama.cpp enforces exact match.

### Key Findings

**1. Alpha-Rising Effect** *(measured)*

Acceptance rate (alpha) increases with output length — the opposite of naive expectation.
Long code generation stabilizes patterns the draft model can predict accurately.

| Context (Gemma 4B→27B Q6_K, n_draft=15, code) | t/s | alpha |
|------------------------------------------------|----:|------:|
| 128 tokens | 49.7 | 84.0% |
| 512 tokens | 51.1 | 88.0% |
| 1024 tokens | 52.7 | 92.6% |
| 16384 tokens | 91.9 | 84.7% |
| 32768 tokens | 108.5 | 91.8% |

*(Hypothesis)* explanation: entropy rate decreases with context (information theory), long texts
repeat patterns (variables, syntax), KV-cache attention fixates on existing tokens → less variance.
**Short benchmarks (128–512 tokens) measure the wrong operating point.**

**2. Temperature Effect** *(measured, Gemma 4B→27B Q6_K, 4k tokens)*

| Domain | temp=0 t/s | temp=1.0 t/s | Delta |
|--------|----------:|-------------:|------:|
| Code | 51.3 | 50.6 | -1% (neutral) |
| Prose | 33.6 | 43.0 | **+28%** |

Temperature **helps prose** substantially (+28%) — at T>0, stochastic acceptance allows
"close enough" tokens through. Code is unaffected (argmax is already highly predictable).
Effect saturates at T≈1.0.

**3. Thinking Model Incompatibility** *(measured)*

Qwen3.5 (thinking mode) achieves alpha 0.5–2% on all tested pairs, including same-source
quantizations and `/no_think` attempts. Root cause: `<think>...</think>` reasoning content
diverges completely between draft and target — CoT paths are inherently unpredictable.
Non-thinking Qwen 3 works normally (alpha=79.4% on code).

**4. MoE Targets Not Profitable** *(measured)*

MoE models (Qwen3 30B-A3B baseline 136.5 t/s) are too fast for draft overhead to pay off.
Best spec result: 93.3 t/s (-32%). Speculative decoding works best on slow dense targets.

### MMQ Performance Cliff Fix (commit bdc2cb438)

**Root cause:** `mul_mat_q_case` in mmq.cuh selects tile width mmq_x to minimize tile count.
At batch=17, it picks mmq_x=24 (one tile) over mmq_x=16 (two tiles). On RDNA3/Wave32,
the 24-wide tile causes occupancy collapse from register pressure — 6x slowdown.

**Fix:** 3 lines — cap mmq_x at 16 for RDNA3.

**Before/After (llama-bench, Gemma 27B Q6_K, single 7900 XTX):**

| pp | Before (t/s) | After (t/s) | Speedup |
|---:|-------------:|------------:|--------:|
| 16 | 248.4 | 251.1 | +1% (unchanged) |
| **17** | **48.6** | **180.4** | **3.71x** |
| 20 | 57.0 | 211.9 | 3.72x |
| 24 | — | 248.3 | smooth |
| 32 | — | 335.4 | smooth |

Post-fix latency curve is smooth (pp=17–32 ≈ 95 ms, two tiles of 16 columns).

**Impact on speculative decoding (Gemma 4B→27B Q6_K, 16k code):**

| n_draft | Without Fix (t/s) | With Fix (t/s) | Delta |
|--------:|------------------:|---------------:|------:|
| 15 | 74.1 | 75.0 | +1% (neutral) |
| **16** | **42.3 (cliff!)** | **55.7** | **+32%** |
| 17 | N/A | 60.1 | new |
| 20 | N/A | 64.9 | new |

The cliff blocked n_draft>15 in practice. Post-fix, n_draft scales smoothly to 20+.

**PPL validation (Llama 3.2 1B, wikitext-2, 5 chunks):** Q4_0 = 13.4486 ± 1.053, Q4_K_M =
12.7761 ± 1.014 — both match stock baselines. Fix only changes tile selection for batch>8,
numerically identical. Q4_K_M pp=17 validated: 3,020 t/s (smooth, no cliff) — fix is
quant-type-independent.
