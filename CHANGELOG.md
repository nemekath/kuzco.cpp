...
- **Speculative decoding benchmarks:** Gemma 4B→27B achieves **91.87 t/s** (+207%), Llama 1B→70B achieves **58.12 t/s** (+153%) using dual-GPU device-pinned configuration
- **MMQ 6x performance cliff fix** at batch>16 on RDNA3 — 3-line fix, zero regression, unlocks n_draft>15 for speculative decoding

### Fixes

- Fix MMQ occupancy collapse at batch=17 on RDNA3: cap mmq_x tile width at 16 to prevent register pressure cliff on Wave32 (`mmq.cuh`)

### Upstream

- Merged 106 commits from llama.cpp upstream (OpenVINO backend, RDNA4 MMVQ params, Nemotron-H NVFP4, flash attention fixes)

### Docs

- Speculative decoding results in `docs/tmac/benchmarks.md`: Config B (dual-GPU device-pinned), model pair compatibility, alpha-rising effect, temperature effects, thinking model incompatibility
